...
function userStreamDiagnostics(streams){
  ...
  const epochLabels=[
    ['soak_epoch_sessions','stream_counter_sessions'],
    ['soak_epoch_order_events','stream_counter_events'],
    ['soak_epoch_planned_reconnects','stream_counter_planned_reconnects'],
    ['soak_epoch_failure_reconnects','stream_counter_failure_reconnects'],
    ['soak_epoch_connection_attempts','stream_counter_attempts'],
    ['soak_epoch_disconnects','stream_counter_disconnects']
  ];
  return streams.flatMap(stream=>{
    const prefix=String(stream.symbol||'').toUpperCase();
    const epoch=epochLabels
      .filter(([key])=>Number(stream[key]||0)>0)
      .map(([key,label])=>`${tr(label)} ${Number(stream[key])}`);
    const lifetime=lifetimeLabels
      .filter(([key])=>Number(stream[key]||0)>0)
      .map(([key,label])=>`${tr(label)} ${Number(stream[key])}`);
    if(stream.last_error) epoch.push(`${tr('stream_counter_error')} ${String(stream.last_error)}`);
    return [
      `${prefix} · ${tr('stream_epoch_metrics')} · ${epoch.join(' · ')||tr('no_data')}`,
      `${prefix} · ${tr('stream_lifetime_metrics')} · ${lifetime.join(' · ')||tr('no_data')}`
    ];
  });
}
...
function updateShadowExperiments(prediction){
  ...
  body.innerHTML=reports.length?reports.map(([symbol,report])=>{
    ...
    if(report.execution_episode&&report.lifecycle_mode==='PROMOTION'){
      const episode=report.execution_episode, validation=episode.model_validation||{}, quality=episode.evidence_quality||{}, next=episode.next_sequential_look==null?NaN:Number(episode.next_sequential_look), terminal=Number(episode.eligible_terminal_episodes||0), rawTerminal=Number(episode.raw_terminal_episodes||terminal), signTrials=Number(episode.nonzero_sign_trials??episode.eligible_terminal_episodes??0), filled=Number(episode.filled_episodes||0), earliestAt=episode.statistics_earliest_possible_ts_ms==null?NaN:Number(episode.statistics_earliest_possible_ts_ms), empiricalAt=episode.statistics_empirical_projected_ts_ms==null?NaN:Number(episode.statistics_empirical_projected_ts_ms), deadlineAt=episode.confirmation_deadline_ts_ms==null?NaN:Number(episode.confirmation_deadline_ts_ms), replayAt=episode.execution_replay_ready_ts_ms==null?NaN:Number(episode.execution_replay_ready_ts_ms), launchAt=episode.expected_launch_ts_ms==null?NaN:Number(episode.expected_launch_ts_ms), confirmed=Array.isArray(episode.confirmed_execution_regimes)?episode.confirmed_execution_regimes.length:0, requiredRegimes=Number(episode.required_confirmed_regimes||0), activation=episode.promotion_eligible===true?'PASS':'BLOCKED';
      const replayText=Number.isFinite(replayAt)?tsShort(replayAt):tr('shadow_not_scheduled');
      const launchLine=Number.isFinite(launchAt)?`<div class="muted">${esc(tr('shadow_expected_launch'))}: ${esc(tsShort(launchAt))}</div>`:'';
      const excursions=episode.excursion_diagnostics||{}, excursionMean=excursions.overall||{}, excursionCount=Number(excursions.filled_episodes||0), mfe=Number(excursionMean.mean_maximum_favorable_excursion_pct), mae=Number(excursionMean.mean_maximum_adverse_excursion_pct);
      const excursionLine=excursions.available===true?`<div class="muted">${esc(tr('shadow_excursions'))}: ${fmt(mfe*100,3)}% / ${fmt(mae*100,3)}% · n=${excursionCount}</div>`:`<div class="muted">${esc(tr('shadow_excursions'))}: —</div>`;
      const entry=episode.entry_quality_diagnostics||{}, economics=entry.candidate_economics||{}, targetReach=Number(entry.target_reachability), breakEven=Number(economics.minimum_break_even_win_rate);
      const entryLine=`<div class="muted">${esc(tr('shadow_entry_quality'))}: ${esc(String(entry.status||'NOT_AVAILABLE'))} · ${Number(entry.completed_filled_paths||0)}/${Number(entry.required_completed_paths||0)} · independent=${Number(entry.independent_filled_paths||0)} · L2=${Number(entry.l2_independent_filled_paths||0)} · ${esc(tr('shadow_target_reach'))}: ${Number.isFinite(targetReach)?fmt(targetReach*100,2)+'%':'—'} · ${esc(tr('shadow_break_even'))}: ${Number.isFinite(breakEven)?fmt(breakEven*100,2)+'%':'—'}</div>`;
      return `<section class="shadow-experiment-symbol"><h3>${esc(symbol)} · ${esc(generation.toUpperCase())} · ${esc(String(report.lifecycle_status||'PRESELECTED'))}</h3><div class="shadow-experiment-row"><div><strong>${esc(String(report.variant_id||'—').replaceAll('_',' '))}</strong><span class="pill ${episode.status==='PASS'?'ok':'warn'}">${esc(String(episode.status||'SHADOW'))}</span></div><div class="muted">${esc(tr('shadow_samples'))}: ${signTrials}/${Number.isFinite(next)?next:signTrials} · episodes: ${terminal} · ${esc(tr('shadow_fill'))}: ${filled}/${terminal} · 360m primary · 300m diagnostic</div><div class="muted">${esc(tr('shadow_statistics_gate'))}: ${esc(String(episode.status||'SHADOW'))} · ${esc(tr('shadow_earliest_eta'))}: ${Number.isFinite(earliestAt)?esc(tsShort(earliestAt)):'—'} · ${esc(tr('shadow_empirical_eta'))}: ${Number.isFinite(empiricalAt)?esc(tsShort(empiricalAt)):'—'} · ${esc(tr('shadow_deadline'))}: ${Number.isFinite(deadlineAt)?esc(tsShort(deadlineAt)):'—'}</div><div class="muted">${esc(tr('shadow_regime_gate'))}: ${confirmed}/${requiredRegimes}</div><div class="muted">${esc(tr('shadow_replay_gate'))}: ${esc(String(validation.status||'BLOCKED'))} · ${esc(replayText)} · ${esc(String(episode.execution_model_rule||'—'))}</div><div class="muted">${esc(tr('shadow_activation_gate'))}: ${esc(activation)}</div><div class="muted">${esc(tr('shadow_evidence_quality'))}: ${esc(String(quality.status||'NOT_AVAILABLE'))} · ${Number(quality.ineligible_terminal_episodes||0)}/${rawTerminal}</div>${excursionLine}${entryLine}${launchLine}<div class="muted">${esc(tr('shadow_waiting_reason'))}: ${esc(String(episode.readiness_reason||'—'))} · active=${episode.active_episode===true?'yes':'no'}</div></div></section>`;
    }
    ...
    const renderVariants=source=>{
      const sourceGeneration=String(source.generation||'');
      const variants=Object.entries(source.variants||{}).filter(([name])=>!sourceGeneration||name.startsWith(`${sourceGeneration}_`));
      return variants.length?variants.map(([name,row])=>{
        ...
        const available=Number(row.available_independent_samples||0), training=Number(row.training_independent_samples||0), requiredTraining=Number(row.required_training_independent_samples||0), historicalTraining=Number(row.historical_training_independent_samples||0), liveTraining=Number(row.live_training_independent_samples||0), evaluated=Number(row.evaluated_independent_samples||0), required=Number(row.required_total_independent_samples||0), readyAt=row.estimated_ready_ts_ms==null?NaN:Number(row.estimated_ready_ts_ms), readyDays=row.estimated_ready_days==null?NaN:Number(row.estimated_ready_days), waiting=String(row.readiness_reason||'');
        const measured=evaluated>0, fill=measured?Number(gate.fill_rate):NaN, low=measured?Number(ci[0]):NaN, high=measured?Number(ci[1]):NaN;
        return `<div class="shadow-experiment-row">
          ...
          <div class="muted">${esc(tr('shadow_raw'))}: ${Number(outcomes.cohort_snapshots||0)} · ${esc(tr('shadow_available'))}: ${available} · ${esc(tr('shadow_training'))}: ${training}/${requiredTraining} (${esc(tr('shadow_historical_training'))}: ${historicalTraining} · ${esc(tr('shadow_live_training'))}: ${liveTraining}) · ${esc(tr('shadow_evaluated'))}: ${evaluated} · ${esc(tr('shadow_required_total'))}: ${required}</div>
          <div class="muted">${esc(tr('shadow_ready_eta'))}: ${Number.isFinite(readyAt)?esc(tsShort(readyAt)):'—'} · ${esc(tr('shadow_ready_days'))}: ${Number.isFinite(readyDays)?fmt(readyDays,1):'—'} · ${esc(tr('shadow_waiting_reason'))}: ${esc(waiting||'—')}</div>
          <div class="muted">${esc(tr('shadow_outcomes'))}: ${Number(outcomes.resolved||0)}/${Number(outcomes.total||0)} · ${esc(tr('shadow_pending'))}: ${Number(outcomes.future||0)+Number(outcomes.settling||0)} · ${esc(tr('shadow_overdue'))}: ${Number(outcomes.overdue||0)}</div>
          <div class="muted">${esc(tr('shadow_fill'))}: ${Number.isFinite(fill)?fmt(fill*100,2)+'%':'—'} · CI: ${Number.isFinite(low)&&Number.isFinite(high)?`${fmt(low,6)}..${fmt(high,6)}`:'—'} · Holm: ${measured?(row.configuration_holm_passed?'PASS':'FAIL'):'—'} · ${esc(tr('shadow_regimes'))}: ${measured?regimes:'—'}/3 · PANIC: ${esc(tr('shadow_safety_only'))}</div>
        </div>`;
    ...
    const confirmationProgress=confirmation.confirmation_progress||{}, regimeBlocks=confirmationProgress.regime_distinct_blocks||{}, requiredRegimeBlocks=Number(confirmationProgress.required_regime_distinct_blocks||0), confirmationEta=Number(confirmationProgress.confirmation_estimated_ready_ts_ms), confirmationDays=Number.isFinite(confirmationEta)?Math.max(0,(confirmationEta-Date.now())/86400000):NaN, confirmationReason=String(confirmationProgress.readiness_reason||'');
    const futile=confirmationProgress.confirmation_futile===true, irrecoverable=Array.isArray(confirmationProgress.irrecoverable_regimes)?confirmationProgress.irrecoverable_regimes:[], executionModel=confirmation.execution_model_gate||{};
    const regimeBlockText=['TREND_UP','TREND_DOWN','RANGE'].map(name=>`${name} ${Number(regimeBlocks[name]||0)}/${requiredRegimeBlocks}`).join(' · ');
    const confirmationLine=`<div class="shadow-experiment-row"><div><strong>${esc(tr('shadow_confirmation'))}</strong><span class="pill ${reportReady?'ok':'warn'}">${esc(String(confirmation.confirmation_status||'BLOCKED'))}</span></div><div class="muted">${esc(tr('shadow_windows'))}: ${Number(confirmation.complete_windows||0)}/${Number(confirmationProgress.required_complete_windows||0)} · ${esc(tr('shadow_samples'))}: ${Number(confirmationProgress.complete_decisions||0)}/${Number(confirmationProgress.required_decisions||0)}</div><div class="muted">${esc(tr('shadow_regime_blocks'))}: ${esc(regimeBlockText)} · ${esc(tr('shadow_confirmation_eta'))}: ${Number.isFinite(confirmationEta)?esc(tsShort(confirmationEta)):'—'} · ${esc(tr('shadow_ready_days'))}: ${Number.isFinite(confirmationDays)?fmt(confirmationDays,1):'—'} · ${esc(tr('shadow_waiting_reason'))}: ${esc(confirmationReason||'—')}</div><div class="muted">${esc(tr('shadow_execution_model'))}: ${esc(String(executionModel.status||'NOT_AVAILABLE'))}${futile?` · ${esc(tr('shadow_futile'))}: ${esc(irrecoverable.join(', ')||'—')}`:''}</div></div>`;
