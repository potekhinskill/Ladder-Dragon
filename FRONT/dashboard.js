  ...
  body.innerHTML=reports.length?reports.map(([symbol,report])=>{
    ...
    const renderVariants=source=>{
      ...
      return variants.length?variants.map(([name,row])=>{
        ...
        const available=Number(row.available_independent_samples||0), training=Number(row.training_independent_samples||0), historicalTraining=Number(row.historical_training_independent_samples||0), evaluated=Number(row.evaluated_independent_samples||0), required=Number(row.required_total_independent_samples||0), readyAt=Number(row.estimated_ready_ts_ms), readyDays=Number(row.estimated_ready_days), waiting=String(row.readiness_reason||'');
        ...
        return `<div class="shadow-experiment-row">
          ...
          <div class="muted">${esc(tr('shadow_raw'))}: ${Number(outcomes.cohort_snapshots||0)} · ${esc(tr('shadow_available'))}: ${available} · ${esc(tr('shadow_training'))}: ${training} (${esc(tr('shadow_historical_training'))}: ${historicalTraining}) · ${esc(tr('shadow_evaluated'))}: ${evaluated} · ${esc(tr('shadow_required_total'))}: ${required}</div>
