...
function updateShadowExperiments(prediction){
  ...
  body.innerHTML=reports.length?reports.map(([symbol,report])=>{
    ...
    if(report.execution_episode&&report.lifecycle_mode==='PROMOTION'){
      const episode=report.execution_episode, validation=episode.model_validation||{}, next=episode.next_sequential_look==null?NaN:Number(episode.next_sequential_look), terminal=Number(episode.eligible_terminal_episodes||0), signTrials=Number(episode.nonzero_sign_trials??episode.eligible_terminal_episodes??0), filled=Number(episode.filled_episodes||0), statsAt=episode.statistics_projected_ready_ts_ms==null?NaN:Number(episode.statistics_projected_ready_ts_ms), regimeAt=episode.regime_projected_ready_ts_ms==null?NaN:Number(episode.regime_projected_ready_ts_ms), championAt=episode.champion_projected_ready_ts_ms==null?NaN:Number(episode.champion_projected_ready_ts_ms);
      return `<section class="shadow-experiment-symbol"><h3>${esc(symbol)} · ${esc(generation.toUpperCase())} · ${esc(String(report.lifecycle_status||'PRESELECTED'))}</h3><div class="shadow-experiment-row"><div><strong>${esc(String(report.variant_id||'—').replaceAll('_',' '))}</strong><span class="pill ${episode.status==='PASS'?'ok':'warn'}">${esc(String(episode.status||'SHADOW'))}</span></div><div class="muted">${esc(tr('shadow_samples'))}: ${signTrials}/${Number.isFinite(next)?next:signTrials} · episodes: ${terminal} · ${esc(tr('shadow_fill'))}: ${filled}/${terminal} · 360m primary · 300m diagnostic</div><div class="muted">${esc(tr('shadow_statistics_eta'))}: ${Number.isFinite(statsAt)?esc(tsShort(statsAt)):'—'} · ${esc(tr('shadow_regime_eta'))}: ${Number.isFinite(regimeAt)?esc(tsShort(regimeAt)):'—'} · ${esc(tr('shadow_champion_eta'))}: ${Number.isFinite(championAt)?esc(tsShort(championAt)):'—'}</div><div class="muted">${esc(tr('shadow_waiting_reason'))}: ${esc(String(episode.readiness_reason||'—'))}</div><div class="muted">${esc(tr('shadow_execution_model'))}: ${esc(String(validation.status||'BLOCKED'))} · ${esc(String(episode.execution_model_rule||'—'))} · active=${episode.active_episode===true?'yes':'no'}</div></div></section>`;
    }
