    ...
    const renderVariants=source=>{
      ...
      return variants.length?variants.map(([name,row])=>{
        ...
        return `<div class="shadow-experiment-row">
          ...
          <div class="muted">${esc(tr('shadow_fill'))}: ${Number.isFinite(fill)?fmt(fill*100,2)+'%':'—'} · CI: ${Number.isFinite(low)&&Number.isFinite(high)?`${fmt(low,6)}..${fmt(high,6)}`:'—'} · Holm: ${measured?(row.configuration_holm_passed?'PASS':'FAIL'):'—'} · ${esc(tr('shadow_regimes'))}: ${measured?regimes:'—'}/4</div>
        </div>`;
