...
- BUY/SELL planning вынесено в `executor_planning.py`: выбор свободных уровней, guard средней цены, расчёт количества и биржевых минимумов теперь тестируются без API и ключей.
- Жизненный цикл долгоживущего исполнителя вынесен в `executor_runtime.py`; модуль управляет длительностью, мягкой остановкой и периодическими runtime-тиками.
- Балансы BUY/SELL уменьшаются только после подтверждённого ответа размещения, поэтому ошибка Binance не резервирует средства только в локальном плане.
