...
### Fixed
- Local and external backup retention now uses exact minute thresholds instead of rounded whole-day thresholds.
- A 14-day local policy no longer retains eligible archives until day 15.

### Security
- Rotation still preserves the newest completed encrypted archive and its checksum.
...
### Fixed
- The known hardware watchdog load gate no longer reboots a responsive host during backup I/O.
- Fresh installs and exact-SHA updates normalize the same known watchdog default.

### Security
- The hardware watchdog device, interval, timeout, and service remain active.
- Other load thresholds, HALT, execution scope, BTC, ETH, and order authority remain unchanged.

### Verified
- The focused deployment suite passes: 96 tests.
- The full project suite passes: 1,511 tests.
- Python compilation, shell syntax, and Technical English checks pass.
...
### Fixed
- Encrypted backup now verifies and mirrors only the archive created by the current run.
- Pre-update backup now uses low CPU and idle I/O scheduling priority.

### Security
- Local and external retention still preserve the newest completed encrypted archive.
- Deployment, HALT, execution scope, BTC, ETH, and order authority remain unchanged.

### Verified
- The focused deployment suite passes: 95 tests.
