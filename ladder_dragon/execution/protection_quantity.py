...
def verify_quantities(journal, parent_id, protection, legs, *, closing=False):
    ...
    if stored:
        expected_ids = {require_order_id(row.get("order_id")) for row in stored}
        if expected_ids != {require_order_id(leg.get("orderId")) for leg in legs}:
            raise RuntimeError("protection legs differ from durable identities")
