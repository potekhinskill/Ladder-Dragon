...
case "${ACTION}" in
  generate)
    [[ "${EUID}" -ne 0 ]] || {
      echo "[FAIL] report generation must use the unprivileged service user" >&2
      exit 1
    }
  ...
  sign)
    [[ "${EUID}" -eq 0 ]] || {
      echo "[FAIL] report signing must run as root" >&2
      exit 1
    }
    [[ -s "${REPORT}" ]] || {
      echo "[FAIL] generated soak report is unavailable" >&2
      exit 1
    }
    ...
    openssl pkeyutl -sign -rawin -inkey "${KEY_FILE}" \
      -in "${REPORT}" -out "${signature_tmp}"
    chmod 0640 "${signature_tmp}"
    mv -f "${signature_tmp}" "${SIGNATURE}"
    echo "[OK] signed soak report generated"
    ;;
  *)
    echo "[FAIL] expected generate or sign" >&2
    exit 2
    ;;
esac
