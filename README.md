...
Исполняемые CLI-команды находятся в `bin/` и запускаются как
`python -m bin.<команда>`. Корень содержит только конфигурацию, документацию и
пакетную структуру; systemd и Raspberry installer используют те же стабильные пути.
...
состояние проверяется и сбрасывается отдельно: `python -m bin.risk_ctl status --testnet`
и `python -m bin.risk_ctl reset --testnet --force`.
...
```bash
python -m bin.ai_advisor_smoke --provider deepseek
```
...
python -m bin.ai_supervisor --help
python -m pytest
```
...
```bash
python -m bin.ai_supervisor \
  --testnet \
  --symbols SOLUSDT,ETHUSDT \
  --base-script ./bin/autosize_universal.py
```

Для отправки ордеров в Testnet одновременно нужны `--live` и точное подтверждение:

```bash
BOT_LIVE_CONFIRMED=YES python -m bin.ai_supervisor \
  --live --testnet \
  --symbols SOLUSDT,ETHUSDT \
  --base-script ./bin/autosize_universal.py
```
...
python -m bin.binance_testnet_smoke --mode order-test --symbol SOLUSDT
```
...
python -m bin.binance_testnet_smoke --mode limit-cancel --symbol SOLUSDT
```
...
python -m bin.binance_testnet_smoke --mode buy-oco --symbol SOLUSDT
```
...
```bash
python -m bin.binance_testnet_smoke --mode circuit-drill --symbol SOLUSDT
```
...
```bash
python -m bin.testnet_soak_monitor --symbol SOLUSDT --duration-sec 43200 \
...
```bash
python -m bin.risk_ctl status
python -m bin.risk_ctl reset
...
Исходный код распространяется по [MIT License](LICENSE). Ограничения гарантий
и ответственности дополнительно описаны в [DISCLAIMER.md](DISCLAIMER.md).
Перед публикацией репозитория необходимо удалить секреты, реальные backup-файлы,
production-конфигурацию и проверить историю Git.
