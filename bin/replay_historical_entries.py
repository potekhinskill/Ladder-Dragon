...
def main() -> int:
    ...
    try:
        ...
        if set(request) != (fields if args.context_db else fields | {"context"}):
        ...
        if args.context_db:
            context_evidence = export_context(
                args.context_db, symbol=request["policy"]["symbol"],
                classifier_fingerprint=request["policy"]["classifier_fingerprint"],
                start_ms=request["start_ms"], end_ms=request["end_ms"], cutoff_ms=request["cutoff_ms"],
            )
