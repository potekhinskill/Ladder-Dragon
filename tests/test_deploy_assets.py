...
def test_mainnet_maker_validation_is_one_shot_and_not_preconfigured():
    ...
    assert "mainnet_stop_limit_validation" in commands
    assert "validate_replay_sessions" in commands
    ...
    stop_source = read(
        "ladder_dragon/verification/live/"
        "mainnet_stop_limit_validation.py"
    )
    assert "python -m bin.mainnet_stop_limit_validation" in runbook
    assert 'HARD_MAX_NOTIONAL_USDT = Decimal("6")' in stop_source
    assert "BOT_MAINNET_STOP_LIMIT_VALIDATION_CONFIRMED" in stop_source
    assert "BOT_MAINNET_STOP_LIMIT_VALIDATION_CONFIRMED" not in example
...
def test_runtime_assets_do_not_manage_unrelated_host_services():
    runtime_assets = read("deploy/install_runtime_assets.sh")

    assert "atop.service" not in runtime_assets
    assert "rtl_tcp.service" not in runtime_assets
...
def test_runtime_dependencies_are_hash_locked_and_installed_without_dependency_resolution():
    ...
    assert "cryptography==50.0.0" in read("requirements/raspberry.lock")
